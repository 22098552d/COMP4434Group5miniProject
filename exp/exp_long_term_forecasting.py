        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                if i % 20 == 0:
                    ...
                    #input_last_col = input[:, -1].reshape(-1, 1)
                    #pred_reshaped = pred[:, -1].reshape(-1, 1)
                    #pred_transposed = pred_reshaped.T
                    #pred_last_col = pred_transposed[0].reshape(-1, 1)
                    #pd = np.concatenate((input_last_col, pred_last_col), axis=0)
